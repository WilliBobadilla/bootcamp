...
import urllib.request        #libreria utilizada para la conexion con la nube de Thingspeak
...
myAPI = '8QWHS6O1IKEOTWOU'         # Acá debe ir el API KEY, dado por la plataforma
...
while True:
	try:
		...
		while temperatura<24 or temperatura>27:  			#si no está en el rango normal, suena una alarma
			gpio.output(buzzer,True)	                    # hacemos sonar una alarma en modo de advertencia
			sleep(4)		 			                    #pausa para el sensor
		...
		sleep(20)                             	# esperamos un tiempo para volver a leer el sensor
